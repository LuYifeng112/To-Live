# TODO: Translation updated at 2020-06-06 00:01

translate chinese strings:

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "We live in the present,\nwe dream of the future\n and we learn eternal truths from the past.\n -Soong Mei Ling"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "We write our own destiny...\nwe become what we do.\n -Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The sky cannot have two suns.\n -Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "If the idea of revolution is to win out,\nit must be through political enlightenment.\nIt is useless to try to impose it by force of arms.\n Sun Yat-Sen"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The whole world is one family.\n -Sun Yat-Sen"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Kindness in words creates confidence.\nKindness in thinking creates profoundness.\nKindness in giving creates love.\n -Mao Ze-Dong"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The people, and the people alone,\n are the motive force in the making of world history.\n -Mao Ze-Dong"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Politics is war without bloodshed\n while war is politics with bloodshed.\n -Mao Ze-Dong"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Who are our enemies? Who are our friends?\n This is a question of the first importance for the revolution.\n -Mao Ze-Dong"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "All diplomacy is a continuation of war by other means.\n -Zhou En-Lai"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "No matter if it is a white cat or a black cat;\nas long as it can catch mice,\nit is a good cat.\n -Deng Xiao-Ping"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Let some people get rich first.\n -Deng Xiao-Ping"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Give me fifty DC-3s\nand the Japanese can have the Burma Road.\n - Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "If imperialism is not banished from the country,\nChina will perish as a nation.\nIf China does not perish, then imperialism cannot remain.\n - Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Politics have no relation to morals.\n -Niccolo Machiavelli"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The more sand has escaped from the hourglass of our life,\nthe clearer we should see through it.\n -Niccolo Marchiavelli"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "It is better to be feared than loved,\n if you cannot be both.\n -Niccolo Machiavelli"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The Chinese people have only family and clan solidarity,\nthey do not have national spirit…\nthey are just a heap of loose sand.\n - Sun Yat Sen"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "The rise or fall of Shanghai\nmeans the birth or death of the whole nation.\n -Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Patriotism demands of us sustained sacrifice.\n -Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Dr. Sun Yat-sen, Father of the Republic,\nmade it his great aim in his revolutionary leadership to secure\nfreedom and equality of status for China\namong the nations of the world.\n -Chiang Kai-Shek"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Human history is in this sense no different from the planet Earth:\nit continues relentlessly on its path,\nwhether at noontime or at midnight.\n -Chen Du-Xiu"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Piercing wind,\nfreezing river of Yi.\nThe hero fords,\nand he never returns!\n -Jing-Ke (failed Assassin of Emperor Qin Shi Huang"
    new ""

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "Amongst the flowers is a pot of wine\n I pour alone but with no friend at hand\n So I lift the cup to invite the shining moon\n Along with my shadow\n a fellowship of three.\n -Li Bai"
    new ""

# TODO: Translation updated at 2020-06-20 17:05

translate chinese strings:

    # game/TO_LIVE_DEFINES/TL_defines_political_menu.rpy:3
    old "A journey of a thousand miles begins with a single step.\n -Lao-Tzu"
    new ""

